...
for TEST in "${TESTS[@]}"; do
    ...
    # 生成反汇编
    $OBJDUMP -d "$TEST_DIR/$TEST.riscv" > "$TEST_DIR/$TEST.riscv.dump"
    
    # 使用 loader.py 生成 hex 文件
    python3 "$UTILS_DIR/loader.py" --fname "$TEST_DIR/$TEST.riscv.dump" --odir "$TEST_DIR"
    
    echo "[OK] $TEST compiled successfully"
    echo "     ELF: $TEST_DIR/$TEST.riscv"
    echo "     Dump: $TEST_DIR/$TEST.riscv.dump"
    echo "     Hex: $TEST_DIR/$TEST.exe"
done
